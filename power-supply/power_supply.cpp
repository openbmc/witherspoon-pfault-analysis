                               ...
                               const std::string& message)
{
    ...
    //constexpr auto ENTRY_INTERFACE = "xyz.openbmc_project.Logging.Entry";
    ...
    try
    {
        ...
        util::getProperty(ASSOCIATION_IFACE, ENDPOINTS_PROPERTY, path,
                          service, bus, logEntries);

        // go through each log entry that matches this callout path
        for (const auto& logEntry : logEntries)
        {
            std::string logEntryService = util::getService(logEntry, 
                                                           LOGGING_IFACE,
                                                           bus);
