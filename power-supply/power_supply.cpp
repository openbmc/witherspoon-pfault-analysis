...
void PowerSupply::checkInputFault(const uint16_t statusWord)
{
    ...
    else
    {
            ...
            !(statusWord & status_word::VIN_UV_FAULT))
        {
            ...
            // The power supply will not be immediately powered on after the
            // input power is restored.
            powerOn = false;
            // Start up the timer that will set the state to indicate we are
            // ready for the powered on fault checks.
            powerOnTimer.start(powerOnInterval, Timer::TimerType::oneshot);
        }
    }
}
