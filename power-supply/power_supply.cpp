...
void PowerSupply::checkInputFault(const uint16_t statusWord)
{
    ...
    {
        log<level::INFO>("INPUT_FAULT_WARN and/or VIN_UV_FAULT on",
...
void PowerSupply::checkCurrentOutOverCurrentFault(const uint16_t statusWord)
{
    ...
    if (outputOCFault < FAULT_COUNT)
    {
        ...
        {
            log<level::INFO>("IOUT_OC_FAULT bit on",
                             entry("STATUS_WORD=0x%04X", statusWord));
            outputOCFault++;
        }
        else
        {
            if (outputOCFault > 0)
            {
                log<level::INFO>("IOUT_OC_FAULT bit off");
                outputOCFault = 0;
            }
        }
...
void PowerSupply::checkTemperatureFault(const uint16_t statusWord)
{
    ...
    if (temperatureFault < FAULT_COUNT)
    {
        ...
        {
            log<level::INFO>("TEMPERATURE_FAULT_WARN and/or OT_FAULT bit on",
                             entry("STATUS_WORD=0x%04X", statusWord),
                             entry("STATUS_TEMPERATURE=0x%04X",
                                   statusTemperature));
            temperatureFault++;
        }
        else
        {
            ...
            {
                log<level::INFO>(
                        "TEMPERATURE_FAULT_WARN and OT_FAULT bit off");
                temperatureFault = 0;
            }
        }
