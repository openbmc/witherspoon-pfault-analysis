...
/**
 * Test the linearToMicroWatts function with different
...
TEST(LinearFormatTest, TestConversions)
{
    ...
    EXPECT_EQ(0, RecordManager::linearToMicroWatts(0));
    EXPECT_EQ(1000000, RecordManager::linearToMicroWatts(1));
    EXPECT_EQ(38000000, RecordManager::linearToMicroWatts(0x26));
    EXPECT_EQ(1023000000, RecordManager::linearToMicroWatts(0x3FF));

    //Mantissa < 0, exponent = 0
    EXPECT_EQ(-1000000, RecordManager::linearToMicroWatts(0x7FF));
    EXPECT_EQ(-20000000, RecordManager::linearToMicroWatts(0x7EC));
    EXPECT_EQ(-769000000, RecordManager::linearToMicroWatts(0x4FF));
    EXPECT_EQ(-989000000, RecordManager::linearToMicroWatts(0x423));
    EXPECT_EQ(-1024000000, RecordManager::linearToMicroWatts(0x400));

    //Mantissa >= 0, exponent > 0
    //M = 1, E = 2
    EXPECT_EQ(4000000, RecordManager::linearToMicroWatts(0x1001));

    //M = 1000, E = 10
    EXPECT_EQ(1024000000000, RecordManager::linearToMicroWatts(0x53E8));

    //M = 10, E = 15
    EXPECT_EQ(327680000000, RecordManager::linearToMicroWatts(0x780A));

    //Mantissa >= 0, exponent < 0
    //M = 0, E = -1
    EXPECT_EQ(0, RecordManager::linearToMicroWatts(0xF800));

    //M = 25, E = -1
    EXPECT_EQ(12500000, RecordManager::linearToMicroWatts(0xF819));

    //M = 100, E = -2
    EXPECT_EQ(25000000, RecordManager::linearToMicroWatts(0xF064));

    //M = 1023, E = -5
    EXPECT_EQ(31968750, RecordManager::linearToMicroWatts(0xDBFF));

    //Mantissa < 0, exponent < 0
    //M = -5, E = -1
    EXPECT_EQ(-2500000, RecordManager::linearToMicroWatts(0xFFFB));

    //M = -1024, E = -11
    EXPECT_EQ(-500000, RecordManager::linearToMicroWatts(0xAC00));
}
...
TEST(ManagerTest, TestRecordAdds)
{
    ...
    {
        EXPECT_EQ(avg * 1000000, std::get<1>(r));
        avg--;
    }
    ...
    {
        EXPECT_EQ(max * 1000000, std::get<1>(r));
        max--;
    }
